...
namespace Dapper.WebAPI.Controllers
{
    ...
    [ApiController]
    public sealed class ProductController(DatabaseService databaseService) : ControllerBase
    {
        [HttpGet]
        ...
        public async Task<IActionResult> Update(UpdateProductDto request)
        {
            ...
            if (result > 0)
            {
                return Ok(new { Message = "Update is success" });
            }
            return BadRequest(new { Message = "Something went wrong..." });
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            if (result > 1)
            {
                return Ok(new { Message = "Delete is success" });
